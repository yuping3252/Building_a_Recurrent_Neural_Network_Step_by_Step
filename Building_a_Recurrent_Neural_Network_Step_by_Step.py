...
def rnn_cell_forward(xt, a_prev, parameters):
    ...
    parameters -- python dictionary containing:
                        Wax -- Weight matrix multiplying the input, numpy array of shape (n_a, n_x)a_next, 
    ...
    #   (n_a,m) <- (n_a,n_x)*(n_x,m) + (n_a,n_a)*(n_a,m) + (n_a,1) 
    #   (n_y,m) <- (n_y,n_a)*(n_a,m) + (n_y,1)
...
def rnn_forward(x, a0, parameters):
    ...
    for t in range(T_x):
        ...
        a_next_, yt_pred, cache = rnn_cell_forward(x[:,:,t], a_next, parameters)
...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    concat = np.concatenate((a_prev, xt), axis=0)       #  (n_a + n_x,m)  <- (n_a,m) , (n_x,m)

    # Compute values for ft, it, cct, c_next, ot, a_next using the formulas given figure (4) (≈6 lines)
    ft = sigmoid(np.dot(Wf,concat) + bf)                           # (n_a,m) <- (n_a,n_a + n_x) (n_a + n_x,m) + (n_a,1)
    it = sigmoid(np.dot(Wi,concat) + bi)                           # (n_a,m) <- (n_a,n_a + n_x) (n_a + n_x,m) + (n_a,1)
    cct= np.tanh(np.dot(Wc,concat) + bc)                           # (n_a,m) <- (n_a,n_a + n_x) (n_a + n_x,m) + (n_a,1)
    c_next = ft*c_prev  + it*cct                                   # (n_a,m) <- (n_a,m)*(n_a,m) + (n_a,m)*(n_a,m) 
    
    ot = sigmoid(np.dot(Wo,concat) + bo)                           # (n_a,m) <- (n_a,n_a + n_x) (n_a + n_x,m) + (n_a,1)
...
def lstm_forward(x, a0, parameters):
    ...
    # print(f"n_x={n_x}, m={m}, T_x={T_x}, n_y={n_y}, n_a={n_a}")
    ...
    # a[4][3][6] =  0.15934505074601704   a0
    # y[1][4][3] = 0.9442297377496516     randn    a0
